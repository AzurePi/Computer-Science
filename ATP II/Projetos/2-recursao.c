...
void decompor(int K, int *num, int *comb, int m, int M, int *quant, int *maior);
//decompõe um inteiro K usando os números no vetor "num", com o endereço de um vetor "comb" que armazenará as combinações
//recebe também o índice m por onde começar a analisar "num" e o tamanho M de "num", além da quantidade, no endereço "quant", de decomposições já feitas
//e o endereço do vetor "maior" que armazena a maior combinação já encontrada
...
int main(){
	...
	int *comb; //endereçp do vetor que armazenará uma combinação de "num"
	int *maior; //endereço do vetor que armazenará a maior combinação possível que decompõe K
	int *quant; //ponteiro para a quantidade de decomposições de um dado número de "inteiro"
	
	scanf("%d %d", &N, &M);
	
	//alocação dinâmica dos vetores de acordo com a entrada
	num = malloc(M*sizeof(int) + 1);
	inteiros = malloc(N*sizeof(int));
	comb = malloc(M*sizeof(int) + 1);
	maior = malloc(M*sizeof(int) + 1);
	...
	for(i = 0; i<N; i++)
	{
		*quant = 0; //zera a quantidade de combinações
		decompor(inteiros[i], num, comb, 0, M+1, quant, maior);
...
//determina o úmtimo índice de um vetor terminado em 0
int size(int *vet, int i){
...
void decompor(int K, int *num, int *comb, int m, int M, int *quant, int *maior){
	...
	while(i != M) //enquanto não estivermos no fim da análise de "num"
	{
		...
		decompor(K - num[i], num, comb, m+1, M, quant, maior); //decompomos o novo K, decrescido do número analisado, e passamos a analisar "num" da posição m+1
